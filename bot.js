...
client.user.setGame(`&help|By bk7`,"http://twitch.tv/S-F")
  console.log('')
  console.log('')
  console.log('╔[═════════════════════════════════════════════════════════════════]╗')
...
راح يتم تعديل البوت تحديث كل يوم
:rocket:البوت يعمل قرابة 24 ساعة
**
        ***__General orders__***
**
『&server /يعرض لك معلومات عن السيرفر』
『&id / يعرض لك معلومات عنك』
『&tobinv / لمعمل انفايت ( دعوة ) لشخص』
...
『&ct | لانشاء روم كتابي 』
『&cv | لانشاء روم صوتي 』
『&bcrole | برودكاست لربتة معينة 』
『&roles | لرؤية رتب السيرفر 』
『&roomsall | لرؤية جميع رومات السيرفر 』
『&giveaway | قيف اواي 』
**
...
client.on('message', message => {
    if (message.content.startsWith(prefix + 'sug')) {
        ...
        if (!message.guild) return message.reply('**:x: هاذا الامر يتشغل فقط داخل السيرفر**').then(v => {v.react('❌')})
...
client.on('guildMemberAdd', member => {
  ...
  const channel = member.guild.channels.find(ch => ch.name === 'اسم الشات هنا');
 
  if (!channel) return;

  channel.send(`**اهلا وسهلا نورت السيرفر ياقلبي :two_hearts: :fire: [user] **
**__استمتع بل سيرفر نقي الي يعجبك من المودات__**
, ${member}`);
});
...
client.on('message',message =>{
  ...
  .setThumbnail("https://f.top4top.net/p_1142nzvoz1.jpg")
           message.channel.send({ embed: embed });
...
if (message.content === '&invite') {
      message.author.send('  https://discordapp.com/api/oauth2/authorize?client_id=536877161550708766&permissions=0&scope=bot  |  تفضل ربط البوت     ').catch(e => console.log(e.stack));
